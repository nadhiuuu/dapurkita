...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-4">
    <form action="{{ route('admin.recipes.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        <div>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama resep..."
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                @forelse ($recipes as $recipe)
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="py-3 px-4">
                            @if ($recipe->image)
                                <img src="{{ Storage::url($recipe->image) }}" class="w-12 h-12 rounded-lg object-cover" alt="{{ $recipe->title }}">
                            @else
                        ...
                        <td class="py-3 px-4 font-medium text-slate-800">{{ $recipe->title }}</td>
                        <td class="py-3 px-4 text-slate-500">
                            <div class="truncate max-w-[150px]">{{ $recipe->description ?? '-' }}</div>
                        </td>
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('admin.recipes.status', $recipe) }}" method="POST">
                                    @csrf
                                    @method('PATCH')

                                    <button type="submit"
                                        title="{{ $recipe->status === 'publish' ? 'Tarik dari tayang' : 'Publikasikan' }}"
                                        class="p-1.5 text-slate-500 hover:text-orange-500 rounded hover:bg-orange-50 transition-colors">
                                        <i data-lucide="{{ $recipe->status === 'publish' ? 'eye-off' : 'eye' }}" class="w-4 h-4"></i>
                                    </button>
                                </form>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="py-6 text-center text-slate-400">Tidak ada resep ditemukan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
