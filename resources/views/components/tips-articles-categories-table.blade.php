@props([
    'tipsArticlesCategories',
])

<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-4">
    <form action="{{ route('admin.tips-articles-categories.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                @forelse ($tipsArticlesCategories as $category)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="py-3 px-4">
                            <span class="px-2 py-1 bg-slate-100 text-slate-700 text-xs font-medium rounded-md">{{ $category->tips_articles_count }} Artikel</span>
                        </td>
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.tips-articles-categories.edit', $category) }}"
                                    class="p-1.5 text-slate-500 hover:text-orange-500 rounded hover:bg-orange-50 transition-colors">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                </a>

                                <form action="{{ route('admin.tips-articles-categories.destroy', $category) }}" method="POST"
    ...
    <x-pagination :paginator="$tipsArticlesCategories" />
</div>
