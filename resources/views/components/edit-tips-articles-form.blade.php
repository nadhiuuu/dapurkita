<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
    <form action="/admin/artikel/1" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label for="judul" class="block text-sm font-semibold text-slate-700 mb-1">Judul Artikel</label>
            <input type="text" id="judul" name="judul" value="5 Cara Mengawetkan Daging Tanpa Kulkas" required
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select id="kategori" name="kategori" required
                    class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-orange-500 transition-all">
                    <option value="tips-dapur" selected>Tips Dapur</option>
        ...
        <div>
            ...
            <div class="mt-2">
                <img id="img-preview" src="https://images.unsplash.com/photo-1556910103-1c02745aae4d?w=200&h=200&fit=crop" class="w-32 h-32 object-cover rounded-lg border border-slate-200" alt="Thumbnail Artikel">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea id="isi" name="isi" rows="10" required
                class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-orange-500 transition-all">Mengawetkan daging tanpa kulkas bisa dilakukan dengan beberapa metode tradisional seperti penggaraman (curing), pengasapan, atau pengeringan. Metode ini dapat meminimalisir pertumbuhan bakteri secara alami...</textarea>
        </div>
