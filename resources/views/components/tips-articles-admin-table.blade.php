...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-4">
    <form action="{{ route('admin.tips-articles.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        <div>
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                @forelse ($tipsArticles as $tipsArticle)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="py-3 px-4">{{ $tipsArticle->user->name ?? '-' }}</td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('admin.tips-articles.status', $tipsArticle) }}" method="POST">
                                    @csrf
                                    @method('PATCH')

                                    <button type="submit"
                                        title="{{ $tipsArticle->status === 'publish' ? 'Tarik dari tayang' : 'Publikasikan' }}"
                                        class="p-1.5 text-slate-500 hover:text-orange-500 rounded hover:bg-orange-50 transition-colors">
                                        <i data-lucide="{{ $tipsArticle->status === 'publish' ? 'eye-off' : 'eye' }}" class="w-4 h-4"></i>
                                    </button>
                                </form>

                                <a href="{{ route('admin.tips-articles.edit', $tipsArticle) }}"
                                    class="p-1.5 text-slate-500 hover:text-orange-500 rounded hover:bg-orange-50 transition-colors">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                </a>

                                <form action="{{ route('admin.tips-articles.destroy', $tipsArticle) }}" method="POST"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="py-6 text-center text-slate-400">Tidak ada artikel ditemukan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
